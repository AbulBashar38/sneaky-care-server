...
client.connect(err => {
    const ProductCollection = client.db(`${process.env.DB_NAME}`).collection("allProduct");
    ...
    const bookingAppointmentCollection = client.db(`${process.env.DB_NAME}`).collection("bookingAppointment");
    ...
    app.post('/addAppointment', (req, res) => {
        bookingAppointmentCollection.insertOne(req.body)
            .then(result => {
                res.send(result.insertedCount > 0)
            })
    })
    ...
    app.get('/getAppointment', (req, res) => {
        bookingAppointmentCollection.find({ email: req.query.email })
            .toArray((err, document) => {
                res.send(document)
            })
    })
    ...
    app.get('/bookingAppointment', (req, res) => {
        bookingAppointmentCollection.find({})
            .toArray((err, document) => {
                res.send(document)
            })
    })
        ...
        orderCollection.updateOne({ _id: ObjectID(req.params.id) },
            ...
            .then(result => {
                if (result.modifiedCount === 0) {
                    bookingAppointmentCollection.updateOne({ _id: ObjectID(req.params.id) },
                        {
                            $set: { status: req.body.newStatus }
                        })
                }
